...
func CreateCSVTemplate(c *gin.Context, ID int) ([]string, int, *rdb.Form) {
...
func GetCreateCSVForAllForms(c *gin.Context) {
	formattedDates, _, _ := CreateCSVTemplate(c, 1)
	...
	CreateExcel(c, "all", dateReservations, formattedDates)
	DownloadExcel("all", c)
}
...
func GetCreateCSVForOneForm(c *gin.Context) {
	...
	formattedDates, ID, form := CreateCSVTemplate(c, ID)
	dateReservations := make(map[string][]ReservationInfo)
	...
	CreateExcel(c, form.Name, dateReservations, formattedDates)
	DownloadExcel(form.Name, c)
}

func CreateExcel(c *gin.Context, formName string, dateReservations map[string][]ReservationInfo, dates []string) {
	// 新しいExcelファイルを作成
	file := xlsx.NewFile()
	...
	// Excelファイルを保存
	err = file.Save("csvFiles/" + formName + "_reservations.xlsx")
	if err != nil {
		log.Fatal(err)
	}
}
